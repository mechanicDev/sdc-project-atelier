  ...
  U.questionsIdHelpful(req.params.question_id)
    .then((data) =>
      res
        .status(200)
        .send(
          `Helpful count has been updated for question Id ${req.params.question_id}`
        )
    )
  ...
  U.answersIdHelpful(req.params.id)
    .then((data) =>
      res
        .status(200)
        .send(
          `Helpful count has been updated for answer ID ${req.params.answer_id}`
        )
    )
    .catch((error) => res.status(400).send(error));
});
...
router.put("/questions/:question_id/report", (req, res) => {
  ...
  U.questionIdReport(req.params.question_id)
    .then((data) =>
      res
        .status(200)
        .send(`Question_Id ${req.params.question_id} has been updated`)
    )
  ...
  U.answersIdReported(req.params.answer_id)
    .then((data) =>
      res
        .status(200)
        .send(
          `Reported count has been updated for answer ID ${req.params.answer_id}`
        )
    )
    .catch((error) => res.status(400).send(error));
});
